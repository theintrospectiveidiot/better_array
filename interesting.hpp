...
int *init_with_name(int mode,int dim,int *shape,...) {
	god_stuff *headr = (god_stuff *)malloc(sizeof(god_stuff) + (sizeof(int)*CAPACITY));  
	headr->make_sure = 28602529;
	headr->count = 0;	
	headr->dim = dim;	
	headr->total = 1;														
	headr->traverse = (int *)malloc(sizeof(int)*headr->dim);	
	headr->strides = (int *)malloc(sizeof(int)*headr->dim);	
	...
	if(headr->total > headr->capacity) {					
		...
		headr =(god_stuff *) realloc(headr,sizeof(god_stuff) + (sizeof(int)*headr->capacity));	
	}										
